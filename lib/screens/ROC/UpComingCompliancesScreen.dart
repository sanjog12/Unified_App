...
class UpComingCompliancesScreenForROC extends StatefulWidget {
  ...
  UpComingCompliancesScreenForROC(
      {Key key, this.client, this.agmRecorded, this.stringAGMDate})
      : super(key: key);

  @override
  _UpComingCompliancesScreenForROCState createState() =>
      _UpComingCompliancesScreenForROCState();
}

class _UpComingCompliancesScreenForROCState
    extends State<UpComingCompliancesScreenForROC> {
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              Text(
                'Tap to Enter SRN & Date of Filing',
                style: TextStyle(
                      ...
                      widget.agmRecorded
                          ? Container(
                              ...
                              child: Column(
                                children: <Widget>[
                                  Text(
                                    'AGM Conclusion Date : ${widget.stringAGMDate}',
                                    style: TextStyle(
                                      fontWeight: FontWeight.bold,
                                      fontSize: 25,
                                    ),
    ...
    try {
      if (rocFromKey.currentState.validate()) {
        ...
        try {
          ...
          flutterToast(
              message:
                  i != 1 ? "Date has Been Recorded" : "Date has been updated");
  ...
  Future<void> setUpComingCompliances() async {
    ...
    try {
      await NotificationServices()
          .deleteNotification('${widget.client.key}1001');
      await updateDBUpcoming(
          'Form ADT-1',
          DateChange.addDayToDate(convertDate(selectedDateOfAgmSubmission), 15)
              .split('-')[1],
          DateChange.addDayToDate(convertDate(selectedDateOfAgmSubmission), 15)
              .split('-')[0]);
      print("writing ");
      NotificationServices().reminderNotificationService(
          '${widget.client.key}1001',
          "${widget.client.name}'s Form ADT-1(ROC)",
      ...
      await NotificationServices()
          .deleteNotification('${widget.client.key}1002');
      await updateDBUpcoming(
          'From AOC-4 & AOC-4 CFS',
          DateChange.addDayToDate(convertDate(selectedDateOfAgmSubmission), 30)
              .split('-')[1],
          DateChange.addDayToDate(convertDate(selectedDateOfAgmSubmission), 30)
              .split('-')[0]);
      NotificationServices().reminderNotificationService(
          '${widget.client.key}1002',
          "${widget.client.name}'s From AOC-4 & AOC-4 CFS(ROC)",
      ...
      await NotificationServices()
          .deleteNotification('${widget.client.key}1003');
      await updateDBUpcoming(
          'Form AOC-4(XBRL)',
          DateChange.addDayToDate(convertDate(selectedDateOfAgmSubmission), 30)
              .split('-')[1],
          DateChange.addDayToDate(convertDate(selectedDateOfAgmSubmission), 30)
              .split('-')[0]);
      NotificationServices().reminderNotificationService(
          '${widget.client.key}1003',
          "${widget.client.name}'s Form AOC-4(XBRL)(ROC)",
          "${widget.client.name}'s Form Form AOC-4(XBRL)(ROC) is on  ${DateChange.addDayToDate(convertDate(selectedDateOfAgmSubmission), 30)}",
          selectedDateOfAgmSubmission.add(Duration(days: 30)));

      await NotificationServices()
          .deleteNotification('${widget.client.key}1004');
      await updateDBUpcoming(
          'Form MGT-7',
          DateChange.addDayToDate(convertDate(selectedDateOfAgmSubmission), 60)
              .split('-')[1],
          DateChange.addDayToDate(convertDate(selectedDateOfAgmSubmission), 60)
              .split('-')[0]);
      NotificationServices().reminderNotificationService(
          '${widget.client.key}1004',
          "${widget.client.name}'s Form MGT-7(ROC)",
      ...
      await NotificationServices()
          .deleteNotification('${widget.client.key}1005');
      await updateDBUpcoming(
          'Form CRA-4',
          DateChange.addDayToDate(convertDate(selectedDateOfAgmSubmission), 30)
              .split('-')[1],
      ...
      await updateDBUpcoming(
          ...
          DateChange.addDayToDate(convertDate(selectedDateOfAgmSubmission), 30)
              .split('-')[0]);
      NotificationServices().reminderNotificationService(
          '${widget.client.key}1006',
          "${widget.client.name}'s Form MGT-14(ROC)",
