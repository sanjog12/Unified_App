...
HashMap<String, MutualFundDetailObject> storedNav =
    HashMap<String, MutualFundDetailObject>();
...
class _MfRecordsState extends State<MfRecords> {
  ...
  Future<void> totalUnitCalculator() async {
    ...
    for (int index = 0; index < history.length; index++) {
      ...
      if (history[index].type == "Lump sum") {
        // print('lump sum');
        double totalUnite = double.parse(history[index].amount) /
            double.parse(history[index].mutualFundDetailObject.nav);
        print(totalUnite);
        setState(() {
  ...
  Future<void> getMfRecords() async {
    // print("Calling");
    history =
        await HistoriesDatabaseHelper().getHistoryOfFMRecordTry(widget.client);
    setState(() {
      gotHistory = true;
    });
  }
  ...
  Future<void> getDeletedDateData(int index) async {
    deletedDateList = await SingleHistoryDatabaseHelper()
        .getDeletedRecordDates(widget.client, history[index].keyDate);
    ...
    temp = await getNAV(
        history[index].mutualFundObject.code,
        history[index].mutualFundDetailObject.date,
        iterations,
        deletedDateList,
        history[index].sipFrequency);
...
//		temp.removeLast();
                                ...
                                ? ListTile(
                                    title: Column(
                                      ...
                                      children: <Widget>[
                                        Container(
                                          height: 10,
                                          color: listOfTotalUnits[index] !=
                                                  'null'
                                              ? (double.parse(listOfTotalUnits[
                                                                  index]) *
                                                              double.parse(
                                                                  history[index]
                                                                      .todayNAV
                                    ...
                                    subtitle: Column(
                                      ...
                                      children: <Widget>[
                                        ...
                                        history[index].mutualFundObject.name !=
                                                "No Record Found"
                                            ? Row(
                                                mainAxisAlignment:
                                                    MainAxisAlignment
                                            ...
                                            ? Row(
                                                children: <Widget>[
                                                  ...
                                                  listOfTotalUnits[index] !=
                                                          'null'
                                                      ? Text(
                                                          (double.parse(listOfTotalUnits[index]) *
                                                                          double.parse(history[index]
                                                                              .todayNAV
                                                                              .nav)) -
                                                                      period[
                                                                          index] <
                                                              ...
                                                              : "+" +
                                                                  ((double.parse(listOfTotalUnits[index]) * double.parse(history[index].todayNAV.nav)) -
                                                                      ...
                                                                      .toStringAsFixed(
                                                                          3),
                                                          style: TextStyle(
                                                              color: (double.parse(listOfTotalUnits[index]) *
                                                                              double.parse(history[index].todayNAV.nav)) -
                                                                          period[index] <
                                                                      0
                                                                  ? Colors.red
                                                                  : Colors.green,
                                    ...
                                    onTap: () {
                                      if (period[index] == null) {
                                        flutterToast(
                                            message:
                                                "Your portfolio is being calculated ... please wait");
                                        return null;
                                      }
                                      Navigator.push(
                                        context,
  ...
  Future<List<MutualFundDetailObject>> getNAV(String code, String actualDate,
      int maxIteration, List<String> deletedList, String sipFrequency) async {
    String tempDate;
    ...
    while (mfDetails != null) {
      if (countIteration >= maxIteration) break;
      tempDate = actualDate;

      if (!storedNav.containsKey(tempDate + code)) {
        for (int i = 0; i < 4; i++) {
          // print(tempDate);
          mfDetails = await MutualFundHelper()
              .getMutualFundNAV(code, tempDate, actualDate);
          if (mfDetails != null)
            break;
          else
        ...
        listMfDetails.add(MutualFundDetailObject(
            date: storedNav[tempDate + code].date,
            nav: storedNav[tempDate + code].nav));
      }
