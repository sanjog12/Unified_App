...
class _RecordDetailState extends State<RecordDetail> {
                      ...
                      onPressed: () async {
                        try {
                          ...
                          await SingleHistoryDatabaseHelper().deletRecordMF(
                              widget.historyForMF.keyDate,
                              widget.client,
                              mutualFund[index].date);
              ...
              Column(
                ...
                children: <Widget>[
                  ...
                  Column(
                    ...
                    children: <Widget>[
                      ...
                      Container(
                        ...
                        decoration: fieldsDecoration,
                        child: Text(
                          widget.historyForMF.mutualFundObject
                              .numberOfInstalments,
                          style: TextStyle(
                            color: whiteColor,
                          ),
                  ...
                  Column(
                    ...
                    children: <Widget>[
                      Text(
                          "NAV value on ${widget.historyForMF.mutualFundDetailObject.date}"),
                      SizedBox(
                        height: 10,
                      ),
                      Container(
                        ...
                        decoration: fieldsDecoration,
                        child: Text(
                            '${widget.historyForMF.mutualFundDetailObject.nav}'),
                      )
                    ],
                  ),
                  SizedBox(
                  ...
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.stretch,
                    children: <Widget>[
                      Text("NAV value on ${widget.historyForMF.todayNAV.date}"),
                      SizedBox(
                        height: 10,
                      ),
                      Container(
                        padding: EdgeInsets.all(15),
                        decoration: fieldsDecoration,
                        child: Text('${widget.historyForMF.todayNAV.nav}'),
                      )
                    ],
                  ),
                  SizedBox(
                  ...
                  Column(
                    ...
                    children: <Widget>[
                      ...
                      Container(
                        padding: EdgeInsets.all(15),
                        decoration: fieldsDecoration,
                        child: Text('${widget.totalInvestment}'),
                      )
                    ],
                  ),
                  SizedBox(height: 30),
                          ...
                          children: <Widget>[
                            ...
                            Text(
                              double.parse(widget.currentValue) -
                                          double.parse(widget.totalInvestment) <
                                      0
                                  ? ''
                                  : '+',
                              style: TextStyle(
                                fontSize: 12,
                                color: double.parse(widget.currentValue) -
                                            double.parse(
                            ...
                            Text(
                              '${(double.parse(widget.currentValue) - double.parse(widget.totalInvestment)).toStringAsFixed(4)}',
                              style: TextStyle(
                                fontSize: 12,
                                color: double.parse(widget.currentValue) -
                                            double.parse(
                  ...
                  widget.historyForMF.type == 'SIP'
                      ? FutureBuilder<List<MutualFundDetailObject>>(
                          future: getNAV(),
                          builder: (BuildContext context,
                              AsyncSnapshot<List<MutualFundDetailObject>>
                                  snapShot) {
                            if (snapShot.hasData) {
                              return Column(
                                crossAxisAlignment: CrossAxisAlignment.stretch,
                                children: <Widget>[
                                  SingleChildScrollView(
                                    child: DataTable(
                                      ...
                                      rows: mutualFund
                                          .asMap()
                                          .map((i, mutual) => MapEntry(
                                              i,
                                              DataRow(
                                                  selected: mutualFundTemp
                                                      .contains(mutual),
                                                  cells: [
                                                    DataCell(
                                                      Text(mutual.date),
  ...
  Future<List<MutualFundDetailObject>> getNAV() async {
    ...
    deletedDateList = await SingleHistoryDatabaseHelper()
        .getDeletedRecordDates(widget.client, widget.historyForMF.keyDate);
    iterations =
        int.parse(widget.historyForMF.mutualFundObject.numberOfInstalments);
    ...
    while (mutualFundDetailObject != null) {
      ...
      for (int i = 0; i < 3; i++) {
        if (widget.storedNav.containsKey(tempDate + code)) {
          ...
          mutualFundDetailObject = await MutualFundHelper()
              .getMutualFundNAV(code, tempDate, startDate);
        }
